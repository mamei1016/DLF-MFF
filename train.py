...
def predict(model,data,batch_size,scaler,args):
    ...
    for i in range(0,data_total,batch_size):
        ...
        with torch.no_grad():
            #pred_now,fpn_out, gcn_out,egnn_out,image_out = model(smile)
            pred_now= model(args,smile)
...
def fold_train(args, log):
    ...
    optimizer = Adam(params=model.parameters(), lr=args.init_lr, weight_decay=args.weight_decay)
